...
@tool
def analyze_documentation(base_path: str, file_path: str) -> Optional[DocumentationAnalysis]:
    """
    ...
    try:
        ...
        # Parse file content
        docstrings = _extract_docstrings(content)
        readme_content = _find_related_readme(file_path)
        
        # Analyze documentation
        coverage = _calculate_coverage(content, docstrings)
        quality = _assess_quality(docstrings)
        missing = _identify_missing_docs(content, docstrings)
        suggestions = _generate_suggestions(coverage, quality, missing)
        
        # Update knowledge graph
        knowledge_graph = nx.DiGraph()
        _update_knowledge_graph(file_path, docstrings, readme_content, knowledge_graph)
...
@tool
def generate_documentation(base_path: str, file_path: str, output_format: str = 'markdown') -> Optional[str]:
    """
    ...
    try:
        ...
        # Extract information
        docstrings = _extract_docstrings(content)
        structure = _analyze_file_structure(content)
        examples = _extract_examples(content)
        
        # Generate documentation
        doc = _format_documentation(
            file_path=file_path,
            structure=structure,
...
def _extract_docstrings(content: str) -> Dict[str, str]:
...
def _find_related_readme(file_path: str) -> Optional[str]:
    """Find and parse related README files"""
    dir_path = os.path.dirname(os.path.join(base_path, file_path))
...
def _calculate_coverage(content: str, docstrings: Dict[str, str]) -> float:
...
def _assess_quality(docstrings: Dict[str, str]) -> float:
...
def _identify_missing_docs(content: str, docstrings: Dict[str, str]) -> List[str]:
    """Identify missing documentation"""
    missing = []
    # Implementation would identify functions/classes without docstrings
    return missing

def _generate_suggestions(coverage: float, quality: float, missing: List[str]) -> List[str]:
...
def _update_knowledge_graph(file_path: str, docstrings: Dict[str, str], readme: Optional[str], knowledge_graph: nx.DiGraph):
    ...
    knowledge_graph.add_node(file_node, type='file')
    
    for name, doc in docstrings.items():
        knowledge_graph.add_node(name, type='docstring')
        knowledge_graph.add_edge(file_node, name)

def _analyze_file_structure(content: str) -> Dict:
...
def _format_documentation(file_path: str, structure: Dict,
                        docstrings: Dict[str, str], examples: List[str],
                        output_format: str) -> str:
    """Format documentation in specified output format"""
    if output_format == 'markdown':
        return _format_markdown(file_path, structure, docstrings, examples)
    elif output_format == 'rst':
        return _format_rst(file_path, structure, docstrings, examples)
    else:
        raise ValueError(f"Unsupported output format: {output_format}")

def _format_markdown(file_path: str, structure: Dict,
...
def _format_rst(file_path: str, structure: Dict,
               docstrings: Dict[str, str], examples: List[str]) -> str:
